...
module.exports = msgHandler = async(rahman, message) => {
    try {
        ...
        function kotor(ktanya) {
            ...
            for (i = 0, len = b.length; i < len; i++) { if (katakotor.kata.indexOf(b[i]) > -1) return 'ok' }
        };
        ...
        switch (command) {
            ...
            case 'undi': //work
                ...
                if (!isGroupAdmins) return rahman.reply(from, 'Perintah ini hanya untuk admin ya, jadi hubungi admin untuk memulai undian', id);
                ...
                break
                // case 'kirimf':
                //     if (args.length === 1) return rahman.reply(from, 'kirimf namaFile', id)
                //     var filn = body.split(' ')[1].replace('..', '')
                //     try { rahman.sendFile(from, `log/${filn}`, filn, '', id) } catch (e) { rahman.reply(from, `Error gan, mungkin file tidak ada`, id) }
                //     break
            case 'sibuk': //work
            ...
            case 'trans':
                ...
                //                 if (error) { rahman.reply(from, `error pada saat mengecek username \n${error}`, id) };
                //                 if (abc === stdout.replace('\n', '')) { rahman.reply(from, 'Halo kak, username yang kakak buat sudah ada, silahkan masukkan username berbeda', id) } else {
                ...
                //                         if (error) { return rahman.reply(from, `error pada saat membuat config \n${error}`, id); };
                ...
                //                             if (error) return rahman.reply(from, `error gan saat import sertifikat ca nya\n\n${error}`, id);
                ...
                //                                 if (error) return rahman.reply(from, `error gan\n\n${error}`, id);
                //                                 rahman.reply(from, `Halo kak, vpn sudah dibuat\nSilahkan ketik vpn help\nuntuk Tutorial cara menggunakannya\nUntuk melihat Syarat \& dan ketentuan berlaku silahkan ketik\nvpn snk`, id);
                //                                 rahman.sendFile(from, lokasin, `${abc}.ovpn`, '', id)
                //                             })
                //                         })
                //                     })
                //                 }
                ...
                //                         if (error) return rahman.reply(from, `Hem ada yang error nih coba lagi kak`, id);
                //                         rahman.reply(from, `Halo kak, user ${abc} sudah dihapus di server saya. terima kasih sudah mencoba vpn kami`, id)
                //                     })
                //                 } else { rahman.reply(from, `Halo kak, User yang anda ketik tidak ada\n Masukkan user yang benar untuk menghapusnya`, id) }
                //             })
                //         } else { rahman.reply(from, `Halo kak, untuk membuat akun vpn silahkan ketik\n*vpn buat username*\n\nUntuk menghapus user ketik\n*vpn hapus username*\n\nIngat ya, username tidak boleh ada spasi\ndan selalu membaca Syarat dan Ketentuan berlaku, untuk melihatnya ketik\n*vpn snk*`, id) }
                //     } else { rahman.reply(from, `Halo kak, Untuk masalah privasi. Fitur vpn ini hanya untuk chat only.`, id) };
                //     break
            ...
            case 'nama':
                ...
                request.get({ headers: { 'content-type': 'application/x-www-form-urlencoded' }, url: 'http://www.primbon.com/arti_nama.php?nama1=' + req + '&proses=+Submit%21+', }, function(error, response, body) {
                    if (error) return rahman.reply(from, 'error gan, hubungi pemilik bot', id);
                ...
                if (isMedia) {
                    ...
                    try {
                        if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                            const mediaData = await decryptMedia(message, uaOverride)
                            rahman.reply(from, '[WAIT] Sedang di proses⏳ silahkan tunggu ± 1 menit!', id)
                        ...
                        rahman.reply(from, 'error gan', id)
                    }
                }
                break
            case 'donasi':
            ...
            case 'wiki':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (args.length === 1) return rahman.reply(from, 'Kirim perintah *wiki [query]*\nContoh : *wiki asu*', id)
                const query_ = body.slice(5)
                const wiki = await get.get('https://arugaz.herokuapp.com/api/wiki?q=' + query_).json()
                if (wiki.error) {
                    rahman.reply(from, wiki.error, id)
                } else {
                    rahman.reply(from, `${wiki.result.replace('by: Astri-chan BOT','')}`, id)
                }
                break
            ...
            case 'fb':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (args.length === 1) return rahman.reply(from, 'Kirim perintah *fb [linkFb]* untuk contoh silahkan kirim perintah *readme*', id)
                if (!args[1].includes('facebook.com')) return rahman.reply(from, mess.error.Iv, id)
                rahman.reply(from, mess.wait, id)
                const epbe = await fb(args[1])
                rahman.sendFileFromUrl(from, epbe.url, `Cuih${epbe.exts}`, epbe.capt, id)
                break
            case 'creator':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
            ...
            case 'nsfwmenu':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                rahman.reply(from, '1. randomHentai\n2. randomNsfwNeko', id)
                break
            case 'ig':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                rahman.reply(from, 'Maaf fitur ini sedang perbaikan', id)
                    // if (args.length === 1) return rahman.reply(from, 'Kirim perintah *ig [linkIg]* untuk contoh silahkan kirim perintah *readme*')
                    // if (!args[1].match(isUrl) && !args[1].includes('instagram.com')) return rahman.reply(from, mess.error.Iv, id)
                    ...
                    //     if(aa.status===404) return rahman.reply(from, 'Maaf link yang anda kirimkan mungkin bukan type gambar/video, jadi saya tidak mau :)',id)
                    //     if(aa.status===200) return rahman.sendFile(from,res.file,res.nama,donasi,id)
            ...
            case 'igstalk':
                ...
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (args.length === 1) return rahman.reply(from, 'Kirim perintah *igStalk @username*\nConntoh *igStalk @duar_amjay*', id)
                var usrr = body.split(' ')
                axios.get(`https://arugaz.herokuapp.com/api/stalk?username=${usrr}`).then(resp => {
                    if (resp.data.status === false) return rahman.reply(from, 'Foto/Video tidak ada', id)
                    if (resp.data.status === 200) {
                        var biodata = resp.data.biodata.replace('ArugaZ', 'rahman')
                        ...
                        var caps = `Nama: ${naman}\nUsername: ${usrnm}\nBio: ${biodata}\nJumlah Post: ${post}\n${follower}\n${following}\n\n${donasi}`
                        rahman.sendFileFromUrl(from, resp.data.Profile_pic, 'Profile.jpg', caps, id)
                    }
                })
            ...
            case 'anime':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id);
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id);
                if (args.length === 1) return rahman.reply(from, 'Kirim perintah *anime [query]*\nContoh : *anime Naruto*', id);
                ...
                axios.get(`https://api.jikan.moe/v3/search/anime?q=${psna}&page=1`).then(resp => {
                    ...
                    if (resp.status === 200) {
                        ...
                        if (dat.chapters === undefined) { chap = '' };
                        tranlstae(dat.synopsis, 'id').then((result) => { rahman.sendFileFromUrl(from, dat.image_url, dat.title, `Judul: ${dat.title}\nType: ${dat.type}\nScore: ${dat.score}${chap}\n\n${result}\n\n${donasi}`, id) })
                    }
                })
                break
            case 'brainly':
                ...
                if (args.length >= 2) {
                    ...
                    if (jum > 10) return rahman.reply(from, 'Max 10!', id)
                    if (Number(tanya[tanya.length - 1])) {
                        tanya
                    }
                    rahman.reply(from, `➸ *Pertanyaan* : ${tanya.split('.')[0]}\n\n➸ *Jumlah jawaban* : ${Number(jum)}`, id)
                    await BrainlySearch(tanya.split('.')[0], Number(jum), function(res) {
                        res.forEach(x => {
                            if (x.jawaban.fotoJawaban.length == 0) {
                                rahman.reply(from, `➸ *Pertanyaan* : ${x.pertanyaan}\n\n➸ *Jawaban* : ${x.jawaban.judulJawaban}\n`, id)
                            } else {
            ...
            case 'linkgroup':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (!isBotGroupAdmins) return rahman.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
                if (isGroupMsg) {
            ...
            case 'bc':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (!isOwner) return rahman.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            ...
            case 'member':
                ...
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id);
                if (!isGroupMsg) return rahman.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id);
                if (!isGroupAdmins && !isOwner) return rahman.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id);
                try {
            ...
            case 'kickall':
                ...
                if (!isGroupOwner) return rahman.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner group', id)
                if (!isBotGroupAdmins) return rahman.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            ...
            case 'admin':
                ...
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (!isGroupMsg) return rahman.reply(from, 'Fitur ini hanya bisa di gunakan dalam group', id)
                if (!isGroupAdmins) return rahman.reply(from, 'Fitur ini hanya bisa di gunakan oleh admin group', id)
                if (!isBotGroupAdmins) return rahman.reply(from, 'Fitur ini hanya bisa di gunakan ketika bot menjadi admin', id)
            ...
            case 'join':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (!isOwner) return rahman.reply(from, 'Hay kalo mau masukin bot ke grub, hubungi ownernya ya\nwa.me/6282237416678\n\n*TIDAK GRATIS*', id)
            ...
            case 'getses':
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (isGroupMsg) return rahman.reply(from, 'Fitur ini hanya bisa di gunakan dalam chat', id)
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                const sesPic = await rahman.getSnapshot()
                rahman.sendFile(from, sesPic, 'session.png', `${donasi}`, id)
                break
            case 'lirik':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (args.length == 1) return rahman.reply(from, 'Kirim perintah *lirik [optional]*, contoh *lirik aku bukan boneka*', id)
                const lagu = body.slice(6)
                const lirik = await liriklagu(lagu)
                rahman.reply(from, lirik, id)
                break
            ...
            case 'listchannel':
                ...
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                rahman.reply(from, listChannel, id)
                break
            case 'jadwaltv':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                if (!isBlocked) return rahman.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot', id)
                if (args.length === 1) return rahman.reply(from, 'Kirim perintah *jadwalTv [channel]*', id)
                const query = body.slice(9).toLowerCase()
                const jadwal = await jadwalTv(query)
                rahman.reply(from, jadwal, id)
                break
            case 'jadwaltvnow':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                ...
                rahman.reply(from, `Jam : ${jadwalNow.jam}\n\nJadwalTV : ${jadwalNow.jadwalTV}`, id)
                break
            case 'waifu':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                ...
                try {
                    (async function() {
                        ...
                        rahman.sendFileFromUrl(from, tujuh.output_url, 'Waifu.jpg', `Semoga Sesuai expetasi :)\n${donasi}`, id)
                    })()
                } catch (error) {
                    rahman.reply(from, `Hmmmm error gan\n\n${error}`)
                }
                break
            case 'random':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                ...
                if (prm === 'nsfwneko') {
                    axios.get(`${urk}nsfwneko`).then(resp => { rahman.sendFileFromUrl(from, resp.data.url, `NSFWNeko Anime.png`, 'NSFWNeko Anime', id) })
                } else if (prm === 'hentai') {
                    axios.get(`${urk}hentai`).then(resp => { rahman.sendFileFromUrl(from, resp.data.url, `Hentai.png`, 'Hentai Anime', id) })
                } else if (prm === 'hug') {
                    axios.get(`${urk}hug`).then(resp => { rahman.sendFileFromUrl(from, resp.data.url, `hug.png`, 'hug', id) })
                } else if (prm === 'nekonime') {
                    axios.get(`${urk}neko`).then(resp => { rahman.sendFileFromUrl(from, resp.data.url, `NekoAnime.png`, 'Neko Anime', id) })
                } else if (prm === 'trapnime') {
                    axios.get(`${urk}trap`).then(resp => { rahman.sendFileFromUrl(from, resp.data.url, `TrapAnime.png`, 'Trap Anime', id) })
                } else if (prm === 'anime') {
                    axios.get(`${urk}anime`).then(resp => { rahman.sendFileFromUrl(from, resp.data.url, `Randomanime.png`, 'Randomanime', id) })
                }
                break
            case 'neko':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                ...
                rahman.sendFileFromUrl(from, 'http://placekitten.com/' + q3 + '/' + q2, 'neko.png', 'Neko ')
                break
            case 'pokemon':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                ...
                rahman.sendFileFromUrl(from, 'https://assets.pokemon.com/assets/cms2/img/pokedex/full/' + q7 + '.png', 'Pokemon.png', )
                break
            case 'ss':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id);
                if (cek() === 'ok') return rahman.reply(from, maintan, id);
                ...
                namaF = url3.parse(_query).hostname;
                if (namaF === '') { namaF = _query.replace('https://', '').replace('http://', '') };
                if (!_query.match(isUrl)) return rahman.reply(from, mess.error.Iv + "\n\nContoh *ss https://google.com*\natau\nss https://google.com full", id);
                if (args.length === 1) return rahman.reply(from, 'Kirim perintah *ss [web]*\nContoh *ss https://google.com*\natau\nss https://google.com full', id);
                var _pilihann = false;
                if (args.length === 3) { pidd = body.split(' ')[2]; if (pidd === 'full') { _pilihann = true; }; if (pidd === 'pdf') { _pilihann = 'ya' } };
                await ss(_query, _pilihann, namaF).then((result) => {
                    if (body.split(' ')[2] === 'pdf') {
                        rahman.sendFile(from, `./log/${namaF}.pdf`, `${namaF}.pdf`, `${donasi}`, id);
                        exec(`rm ./log/${namaF}.pdf`)
                    } else {
                        rahman.sendFile(from, `./log/${namaF}.png`, `${namaF}.png`, `${donasi}`, id);
                        exec(`rm ./log/${namaF}.png`);
                    }
                }).catch((err) => rahman.reply(from, `Error tidak dapat mengambil screenshot website ${_query}\n\n${err}`, id))
                break
            case 'quote':
            case 'quotes':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
            ...
            case 'quoteanime':
                ...
                skya_ = skya.data
                rahman.reply(from, `➸ *Quotes* : ${skya_.quote}\n➸ *Character* : ${skya_.character}\n➸ *Anime* : ${skya_.anime}`, id)
                break
            case 'meme':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)
                if (cek() === 'ok') return rahman.reply(from, maintan, id)
                ...
                rahman.sendFileFromUrl(from, `${url}`, 'meme.jpg', `${title}\n\n${donasi}`, id)
                break
            case 'menu':
            case 'help':
                if (kotor(body.toLowerCase()) === 'ok') return rahman.reply(from, jagaOmongan, id)

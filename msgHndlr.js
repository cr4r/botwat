...
module.exports = msgHandler = async (CR4R, message) => {
    try {
        ...
        case 'ss':
            ...
            await ss(_query,_pilihann,namaF).then((result) => { if(body.split(' ')[2] === 'pdf'){ CR4R.sendFile(from, `./log/${namaF}.pdf`, `${namaF}.pdf`, `${donasi}`, id); exec(`rm ./log/${namaF}.pdf`) }else{ CR4R.sendFile(from, `./log/${namaF}.png`, `${namaF}.png`, `${donasi}`, id); exec(`rm ./log/${namaF}.png`);} })

...
module.exports = msgHandler = async (CR4R, message) => {
    try {
        ...
        case 'pokemon':
            ...
            break
        case 'ss':
            if(kotor(body.toLowerCase()) === 'ok') return CR4R.reply(from,jagaOmongan,id);if(cek()==='ok') return CR4R.reply(from,maintan,id);if (!isBlocked) return CR4R.reply(from, 'Hey hey orang yang sudah di blok tidak bisa gunakan bot',id);var _query = body.split(' ')[1];namaF = url3.parse(_query).pathname.split('/').pop();if(namaF === ''){ namaF = _query.replace('https://','').replace('http://','') };if (!_query.match(isUrl)) return CR4R.reply(from, mess.error.Iv+"\n\nContoh *ss https://google.com*\natau\nss https://google.com full", id);if (args.length === 1) return CR4R.reply(from, 'Kirim perintah *ss [web]*\nContoh *ss https://google.com*\natau\nss https://google.com full', id);var _pilihann = false;if(args.length === 3) { pidd = body.split(' ')[2]; if(pidd === 'full'){ _pilihann = true; }; if(pidd === 'pdf'){ _pilihann = 'ya'} };await ss(_query,_pilihann,namaF).then((result) => { if(body.split(' ')[2] === 'pdf'){ CR4R.sendFile(from, `./log/${namaF}.pdf`, `${namaF}.pdf`, `${donasi}`, id); exec(`rm ./log/${namaF}.pdf`) }else{ CR4R.sendFile(from, `./log/${namaF}.png`, `${namaF}.png`, `${donasi}`, id); exec(`rm ./log/${namaF}.png`);} }).catch((err) => CR4R.reply(from, `Error tidak dapat mengambil screenshot website ${_query}\n\n${err}`, id))
            break
        case 'quote':
        case 'quotes':
            if(kotor(body.toLowerCase()) === 'ok') return CR4R.reply(from,jagaOmongan,id)
